...
export default async function AdminOrderPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string;message?:string}>}){
  ...
  return <AppShell role="admin"><Notice searchParams={await searchParams}/><header><p className="eyebrow">{data.reference}</p><h1>{one(data.listings)?.title}</h1><div className="actions"><StatusChip value={data.status}/>{escrow&&<StatusChip value={escrow.status}/>}</div><p className="price">{money(data.agreed_price,data.currency)}</p></header>
    ...
    {data.status==="inspection_in_progress"?<><form action={adminAdvanceOrder} className="form"><input type="hidden" name="orderId" value={id}/><input type="hidden" name="operation" value="pass"/><label>Inspection notes<textarea name="notes"/></label><SubmitButton pending="Recording approval…" confirmMessage="Pass verification and make this item eligible for buyer delivery?">✓ Pass verification</SubmitButton></form><form action={adminAdvanceOrder} className="form danger-zone"><input type="hidden" name="orderId" value={id}/><input type="hidden" name="operation" value="fail"/><input type="hidden" name="listingMatches" value="false"/><input type="hidden" name="conditionMatches" value="false"/><label>Failure reason<input name="reason" required/></label><label>Participant-safe summary<textarea name="notes"/></label><SubmitButton className="danger" pending="Recording rejection…" confirmMessage="Fail verification and begin the refund/return branch?">✕ Fail verification</SubmitButton></form></>
    :actions[data.status]?.map(([operation,label])=><form action={adminAdvanceOrder} className="form" key={operation}><input type="hidden" name="orderId" value={id}/><input type="hidden" name="operation" value={operation}/>{operation==="start_delivery"&&<><label>Courier name<input name="courierName" defaultValue="Demo Courier 12"/></label><label>Courier phone<input name="courierPhone"/></label></>}<SubmitButton pending="Updating transaction…" confirmMessage={`${label}? This advances the hosted transaction.`}>{label}</SubmitButton></form>)??<p>No operational action required.</p>}
