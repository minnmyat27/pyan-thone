...
export function ChatThread({ conversationId, userId, initialMessages, editListingHref }: { conversationId:string;userId:string;initialMessages:Message[];editListingHref?:string }) {
  ...
  return <><div className="chat-log" aria-live="polite">{messages.map(message=><article className={message.sender_id===userId?"mine":""} key={message.id}><p>{message.body}</p><small>{new Date(message.created_at).toLocaleString()}</small></article>)}<div ref={end}/></div>
    <p className="notice">Keep payments and conversations inside Pyan Thone. Never share passwords or OTPs.</p><form action={sendMessage} className="chat-compose"><input type="hidden" name="conversationId" value={conversationId}/><label><span>Message</span><textarea name="body" required maxLength={4000} placeholder="Write a message…"/></label><SubmitButton pending="Sending…">Send →</SubmitButton>{editListingHref&&<a className="button-link secondary" href={editListingHref}>Update listing price</a>}</form></>;
}
